...
exercise_07_01_camera_circle :: types.Tableau {
	title = "Revolving camera",
	init = proc() {
	...
	update = proc(delta: f64) {
		...
		time_f32: f32 = f32(time)
		camera.position.x = math.sin(time_f32) * radius
		camera.position.z = math.cos(time_f32) * radius
		camera.direction = linalg.normalize(-camera.position)
	},
	draw = proc() {
