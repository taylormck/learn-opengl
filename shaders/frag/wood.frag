...
void main() {
	...
	vec2 ring = fs_in.tex_coords.xy - 0.5;

	float turbulence = turbulence_power * turbulence(fs_in.tex_coords, max_zoom);
	float z_dist = length(ring.xy) + turbulence;

	float value = 0.5 * abs(sin(2 * ring_frequency * z_dist * 3.14159));

	float red = value + 80.0 / 256.0;
	float green = value + 30.0 / 256.0;
	float blue = 0.0;
